...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc = np.random.randint(4, int(self.block_size*7/8)+1)
        trunc = min(trunc, len(document))

        document_trunc = document[:trunc]
        mask_length = np.random.binomial(trunc, 1/4)
        mask_start = np.random.randint(trunc-mask_length+1)

        prefix = document_trunc[:mask_start]
        masked_content = document_trunc[mask_start:mask_start+mask_length]
        suffix = document_trunc[mask_start+mask_length:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + (self.block_size - trunc - 1) * self.PAD_CHAR
        x = masked_string[:-1]
        y = masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        ...
        # if x.shape[0] != 127:
        #     print(len(doc))
        #     print(len(document_trunc))
        #     print(trunc)
        #     print(mask_start, mask_length)
        #     print(len(prefix), len(suffix), len(masked_content), self.block_size - trunc)
        #     print(x.shape, y.shape)
        return x, y
