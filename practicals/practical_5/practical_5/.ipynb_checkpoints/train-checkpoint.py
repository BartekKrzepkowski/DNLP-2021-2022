...
parser.add_argument(
    '--attention_type', type=str, default='bahdanau')
parser.add_argument(
...
parser.add_argument('--load_param', type=util.str2bool, nargs='?',
...
def train(print_loss_total,print_act_total, print_grad_total, input_tensor,
    target_tensor, bs_tensor, db_tensor, name=None):
    ...
    loss, loss_acts, grad = model.train(
        input_tensor, input_lengths, target_tensor, target_lengths, db_tensor,
        bs_tensor, name)
...
def train_iters(model, n_epochs=10, args=args):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for name in dials:
            ...
            input_tensor, target_tensor, bs_tensor, db_tensor = util.loadDialogue(  # noqa
        ...
        print('Time since %s (Epoch:%d %d%%) Loss: %.4f, Loss act: %.4f, Grad: %.4f' % (
            util.timeSince(
                start, epoch / n_epochs), epoch,
                epoch / n_epochs * 100, print_loss_avg,
                print_act_total_avg, print_grad_avg)
        )
...
if __name__ == '__main__':
    input_lang_index2word, output_lang_index2word, input_lang_word2index, output_lang_word2index = load_dictionaries()
    ...
    model = Model(
        args, input_lang_index2word, output_lang_index2word,
    ...
    train_iters(model, n_epochs=args.max_epochs, args=args)
