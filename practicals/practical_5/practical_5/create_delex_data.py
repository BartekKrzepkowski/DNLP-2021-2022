...
def belief_state(bstate, summary_bstate, domain):
    ...
    # YOUR CODE HERE
    mapping = {'not mentioned': 0, "don't care": 1}
    bstate = bstate[domain]['semi']
    i = 0
    summary_bstate = [0]*12
    for slot in bstate:
        if slot == 'requested':
            continue
        pos = 2 if bstate[slot] not in mapping else mapping[bstate[slot]]
        summary_bstate[3 * i + pos] = 1
        i += 1
...
def requested_state(bstate, summary_bstate, domain):
    ...
    Returns:
        summary_bstate: list
    """
    bs_reqs = bstate[domain]['semi']['requested']
    req = [0] * 7
