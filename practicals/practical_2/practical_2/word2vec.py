...
def naive_softmax_loss_and_gradient(
    ...
    loss = - np.log(y_hat.flatten()[outsideWordIdx])
    oh = np.zeros_like(y_hat)
    oh[outsideWordIdx] = 1
    gradCenterVec = outsideVectors.T @ y_hat - outsideVectors[outsideWordIdx][:, np.newaxis]
    gradOutsideVecs = (y_hat - oh) @ centerWordVec[np.newaxis]
...
def neg_sampling_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    chosen_outside = outsideVectors[np.array(negSampleWordIndices)]
    pos = sigmoid(outsideVectors[outsideWordIdx].T @ centerWordVec)
    negs = sigmoid(- chosen_outside @ centerWordVec)[:, np.newaxis]
    loss = -np.log(pos) - np.log(negs).sum()
    gradCenterVec = ((1 - negs) * chosen_outside).sum(axis=0) - (1 - pos) * outsideVectors[outsideWordIdx]
    R = np.zeros((outsideVectors.shape[0], 1))
    R[outsideWordIdx] = pos - 1
    for i in range(len(negSampleWordIndices)):
        R[negSampleWordIndices[i]] += 1 - negs[i]
    gradOutsideVecs = R @ centerWordVec[np.newaxis]
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outside_idxs = [word2Ind[word] for word in outsideWords]

    for i in outside_idxs:
        a, b, c = word2vecLossAndGradient(centerWordVec, i, outsideVectors, dataset)
        loss += a
        gradCenterVecs[word2Ind[currentCenterWord]] += b.flatten()
